...
COPY packages/fpay/package.json packages/fpay/package.json
...
COPY packages/fpay packages/fpay
COPY packages/fwk packages/fwk
RUN pnpm --filter @getfluxo/fpay build && pnpm --filter @getfluxo/fwk build
