...
RUN npm i -g pnpm@10.33.0 && pnpm install --filter @getfluxo/fwk... --frozen-lockfile --prod=false --ignore-scripts
...
COPY package.json pnpm-lock.yaml pnpm-workspace.yaml .npmrc ./
COPY packages/fwk/package.json packages/fwk/package.json
RUN npm i -g pnpm@10.33.0 && pnpm install --filter @getfluxo/fwk --prod --frozen-lockfile --ignore-scripts
